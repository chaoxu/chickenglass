...
export function useStructureSourceSelectionBridge(
  ...
  return useCallback((selection: MarkdownEditorSelection) => {
    const blockPosition = readSourcePositionFromElement(editor.getElementByKey(nodeKey));
    if (blockPosition === null) {
      return;
    }
    const rawOffset = typeof offsetInRaw === "function"
      ? offsetInRaw(selection)
      : offsetInRaw + selection.anchor;
    if (rawOffset === null) {
      return;
    }
    editor.dispatchCommand(
      SET_SOURCE_SELECTION_COMMAND,
      blockPosition + rawOffset,
    );
