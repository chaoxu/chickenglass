...
describe("createLexicalRenderResourceResolver", () => {
  ...
  it("keeps include-file reads behind the same resolver but requires a source document path", async () => {
    const resolver = createLexicalRenderResourceResolver(new MemoryFileSystem({
      "chapter.md": "# Included",
    }));

    await expect(resolver.readIncludedDocument("chapter.md")).resolves.toBeNull();
  });
});
