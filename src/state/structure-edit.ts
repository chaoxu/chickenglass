...
export function structureEditActive(
  ...
  return { status: "editing", blockKey, variant };
}

export function isStructureEditActive(state: StructureEditState): state is Extract<StructureEditState, { status: "editing" }> {
  return state.status === "editing";
}

export function isSameStructureEdit(a: StructureEditState, b: StructureEditState): boolean {
  if (a.status !== b.status) {
    return false;
  }
  if (a.status === "editing" && b.status === "editing") {
    return a.blockKey === b.blockKey && a.variant === b.variant;
  }
  return true;
}
