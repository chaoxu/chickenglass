...
/**
 * Build a minimal preferences UI for managing editor plugins.
 *
 * Returns a `<div class="editor-plugin-prefs">` element.  The caller is
 * responsible for inserting it into the DOM.  Pass a `getView` callback
 * so the UI can reach the live `EditorView` at toggle time (the view may
 * be replaced when files are switched).
 */
export function buildPluginPrefsUI(
  manager: EditorPluginManager,
  getView: () => EditorView | null,
): HTMLElement {
  const root = document.createElement("div");
  root.className = "editor-plugin-prefs";

  const title = document.createElement("h3");
  title.className = "editor-plugin-prefs-title";
  title.textContent = "Editor Features";
  root.appendChild(title);

  const list = document.createElement("ul");
  list.className = "editor-plugin-prefs-list";
  root.appendChild(list);

  for (const { plugin, enabled } of manager.getPlugins()) {
    const item = document.createElement("li");
    item.className = "editor-plugin-prefs-item";

    const label = document.createElement("label");
    label.className = "editor-plugin-prefs-label";

    const checkbox = document.createElement("input");
    checkbox.type = "checkbox";
    checkbox.className = "editor-plugin-prefs-checkbox";
    checkbox.checked = enabled;
    checkbox.addEventListener("change", () => {
      // setEnabled accepts null view: updates internal state even when no
      // editor is open; the compartment reconfigure is skipped in that case.
      manager.setEnabled(getView(), plugin.id, checkbox.checked);
    });

    const nameSpan = document.createElement("span");
    nameSpan.className = "editor-plugin-prefs-name";
    ...
    if (plugin.description) {
      ...
      desc.className = "editor-plugin-prefs-desc";
