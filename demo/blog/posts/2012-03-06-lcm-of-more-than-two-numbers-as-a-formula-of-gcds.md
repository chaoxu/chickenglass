...
\[
`\lcm`{=tex}(a,b,c) = `\frac{abc}{\gcd(a,b,c)}`{=tex}
\]
It isn't.
...
\[
`\lcm`{=tex}(a,b,c) = `\frac{abc \gcd(a,b,c)}{\gcd(a,b)\gcd(b,c)\gcd(a,c)}`{=tex}
\]
...
\[
`\max`{=tex}(a,b,c) = a+b+c+`\min`{=tex}(a,b,c)-(`\min`{=tex}(a,b)+`\min`{=tex}(b,c)+`\min`{=tex}(a,c))
...
\[`\max`{=tex}(a_1,`\ldots`{=tex},a_n) = `\mu`{=tex}(`\bigcup`{=tex}\_{i=1}\^n \[0,a_i\]).\]
