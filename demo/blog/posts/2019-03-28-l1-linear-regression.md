...
In the linear regression problem, we are given a point set $S\subset \R^2$ of size $n$ and we are interested in find a line $f(x) = ax+b$, such that it minimizes the *error*, defined as

\[
`\sum`{=tex}\_{(x,y)`\in `{=tex}S} \|y - f(x)\|\_p
\]
...
\[
`\sum`{=tex}*{(x,y)`\in `{=tex}S\^+} y - ax-b + `\sum`{=tex}*{(x,y)`\in `{=tex}S\^-} ax+b - y
\]
