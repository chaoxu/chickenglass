...
\[
`\left`{=tex}\|{ (c_1,`\ldots`{=tex},c_m) \| c_i`\in `{=tex}`\N `{=tex}, `\sum`{=tex}\_{i=1}\^m c_i d_i = n} `\right`{=tex}\|.
\]
...
\[
C(x) = `\prod`{=tex}*{i=1}\^m `\sum`{=tex}*{j=0}\^`\infty `{=tex}x\^{j d_i}
\]
...
\[
`\sum`{=tex}\_{k=0}\^`\infty `{=tex}P(x) { k+m-1 `\choose `{=tex}m-1 } x\^{lk}
\]
