...
\[
d_w(G) = `\frac{2\sum_{e\in E} w(e)}{|G|}`{=tex},
\]
and the minimum weighted degree
\[
`\delta`{=tex}*w(G) = `\min`{=tex}*{e`\in `{=tex}E} {w(e)}.
\]
...
\[
`\delta`{=tex}*w(G) `\leq `{=tex}`\sum`{=tex}*{{v_i,v_n}`\in `{=tex}E} w({v_i,v_n}) `\leq `{=tex}`\sum`{=tex}*{i=1}\^{n-1} w({v_i,v*{i+1}}) = W(v_1`\ldots `{=tex}v\_{n})
\]
:::
