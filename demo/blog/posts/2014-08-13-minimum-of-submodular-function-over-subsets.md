...
\[
f_P(X) = `\min`{=tex}\_{Y`\in `{=tex}P(X)} f(Y)\\
\]
is submodular.
:::
...
\[
f\_*(X) = `\min `{=tex}{f(Y)\|Y`\subset `{=tex}X}\\
f\^*(X) = `\min `{=tex}{f(Y)\|X`\subset `{=tex}Y}
\]
...
\[
f_T(X) = `\min `{=tex}{f(Y)\|Y`\subset `{=tex}X, T`\setminus `{=tex}X`\subset `{=tex}V`\setminus `{=tex}Y}\\
\]
is submodular.
:::
