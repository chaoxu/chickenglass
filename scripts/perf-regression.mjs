...
import {
  ...
  printComparison,
  printReportSummary,
} from "./perf-regression-reporting.mjs";
import {
...
export async function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    const reportWithAnswers = {
      ...report,
      answerTable: performanceAnswerRows(report),
    };
    ...
    if (command === "capture") {
      ...
      if (outputPath) {
        ...
        ensureDir(resolved);
        writeFileSync(resolved, JSON.stringify(reportWithAnswers, null, 2) + "\n");
        console.log(`Wrote perf baseline to ${resolved}`);
      }
      printReportSummary(reportWithAnswers);
      return;
    }
    ...
    if (command === "compare") {
      ...
      const comparison = comparePerfRegressionReports(baseline, reportWithAnswers, {
