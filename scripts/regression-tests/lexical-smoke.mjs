...
const MODE_SWITCH_FIXTURE = {
  ...
  content: [
    ...
    "",
    '::: {.definition #def:mode-switch-stability title="Mode Switch Stability"}',
    "A mode switch keeps the canonical markdown snapshot as the source of truth.",
    ":::",
    "",
    '::: {.theorem #thm:mode-switch-stability title="Mode Switch Theorem"}',
...
function createHeavySmokeDoc() {
  const sections = Array.from({ length: 180 }, (_, offset) => {
    ...
    return [
      ...
      "",
      `::: {.theorem #thm:smoke-${index} title="Smoke Theorem ${index}"}`,
