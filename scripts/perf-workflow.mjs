...
export async function main(argv = process.argv.slice(2)) {
  const { command, options } = splitCommand(argv, ["check", "baseline"], "baseline");
  ...
  const scenario = parser.getFlag("--scenario", DEFAULT_SCENARIO);
  const baselinePath = resolve(parser.getFlag("--baseline", DEFAULT_BASELINE));
  const outputPath = resolve(parser.getFlag("--output", DEFAULT_BASELINE));
  const server = await startOrReuseDevServer({ url: parser.getFlag("--url") });
  ...
  try {
    ...
    parser.forwardFlags(perfArgs, ["--iterations", "--warmup", "--threshold-pct", "--min-delta-ms", "--settle-ms"]);
