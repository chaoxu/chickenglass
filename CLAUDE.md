...
- **Verify in browser with screenshots**: After implementing visual changes, ALWAYS take a screenshot and visually inspect it before claiming the fix works. Never rely solely on numerical values (computed styles, bounding rects) — they can look correct while the visual result is wrong. Take the screenshot, look at it, and compare against the expected result before reporting done.
